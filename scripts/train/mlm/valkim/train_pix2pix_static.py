...
def train_pix2pix_static(
    ...
    loader_kwargs = dict(
        ...
        preload_train_data=preload_train_data,
        preload_val_data=preload_val_data,
        # transform_train=transform_train,
    )
    tl, vl = StaticLoader.build_loaders(dataset, pat, **loader_kwargs)
    print(f'[train] train_loader_len={len(tl)} val_loader_len={len(vl)} batch_size={batch_size} num_workers={num_workers} preload_train={preload_train_data} preload_val={preload_val_data}')
    ...
    for e in range(start_epoch, n_epochs):
        ...
        train_iter = iter(tl)
        # print(f'[train] epoch={e} tqdm_total={len(tl)}; the progress bar will show 0/{len(tl)} until the first batch finishes because tqdm is initialized before the iterator advances.')
...
if __name__ == '__main__':
    ...
    train_pix2pix_static(
        ...
        pat=pat_id,
        project='MLM-VALKIM',
        model=f'pix2pix2d-static-{pat_id}-{version}',
        n_epochs=100,
        # lr_init=5e-5,   # Based on LR find result.
        lr_init=2e-4,   # Default pix2pix LR.
        batch_size=32,
        num_workers=4,

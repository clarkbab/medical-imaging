...
from dicomset.utils import dice, distances, save_csv, append_row, region_to_list, crop, load_csv, to_numpy
import numpy as np
import os
import pandas as pd
...
for p in tqdm(pat_ids):
    ...
    fixed_regions, fixed_regions_data = fixed_regions_series.data(r='l:hn')
    ...
    regions = region_to_list('l:hn', region_map=set.region_map)
    for m in models:
        moved_regions, moved_regions_data = model_series[m].data(r='l:hn')
        for r in regions:
...
save_csv(res_df, 'files:imreg/hn-seg.csv')

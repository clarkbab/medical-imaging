from augmed import Pipeline, RandomAffine, RandomElastic
...
def create_static_training_dataset(
    create_train_projections: bool = True,
    ...
    if create_train_volumes:
        for p in tqdm(pat_ids, desc="Creating augmented training volumes for patients"):
            ...
            inh_label_names, inh_labels = inh_regions_series.data(r=regions, rr=True)
            exh_label_names, exh_labels = exh_regions_series.data(r=regions, rr=True)
            ...
            device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
            assert device.type == 'cuda', "CPU processing is very slow - consider using a GPU."
    ...
    if create_train_projections:
        for p in tqdm(pat_ids, desc="Creating train projections for patients"):
            ...
            for i in range(n_train_volumes):
                ...
                # Sample kV detector angles from a uniform distribution.
                # kv_detector_angles = list(np.random.uniform(min_angle, max_angle, n_train_angles))
                # Don't use a uniform distrbution for static dataset - replicate the cGAN
                # project sampling strategy.
                kv_detector_angles = np.linspace(min_angle, max_angle, n_train_angles).tolist()
                ...
                inh_ct_proj = F.interpolate(inh_ct_proj.unsqueeze(1).float(), size=TRAIN_SIZE, mode='bilinear', align_corners=False).squeeze(1)
                inh_labels_proj = F.interpolate(inh_labels_proj.float(), size=TRAIN_SIZE, mode='nearest')
                ...
                exh_ct_proj = F.interpolate(exh_ct_proj.unsqueeze(1).float(), size=TRAIN_SIZE, mode='bilinear', align_corners=False).squeeze(1)
                exh_labels_proj = F.interpolate(exh_labels_proj.float(), size=TRAIN_SIZE, mode='nearest')

                # Save each projection separately.
                for j, angle in enumerate(kv_detector_angles):
                    filepath = os.path.join(projpath, f"inh_ct_{i}_{j}.npy")
                    save_numpy(inh_ct_proj[j], filepath, overwrite=True)
                    filepath = os.path.join(projpath, f"exh_ct_{i}_{j}.npy")
                    save_numpy(exh_ct_proj[j], filepath, overwrite=True)
                    filepath = os.path.join(projpath, f"inh_labels_{i}_{j}.npy")
                    save_numpy(inh_labels_proj[j], filepath, overwrite=True)
                    filepath = os.path.join(projpath, f"exh_labels_{i}_{j}.npy")
                    save_numpy(exh_labels_proj[j], filepath, overwrite=True)
                    filepath = os.path.join(projpath, f"angle_{i}_{j}.json")
                    save_json(angle, filepath, overwrite=True)
    ...
    if create_val_volumes:
        for p in tqdm(pat_ids, desc="Creating augmented validation volumes for patients"):
            ...
            exh_ct = exhale_series.data
            exh_affine = exhale_series.affine
            inh_label_names, inh_labels = inh_reg_series.data(r=regions, return_regions=True)
            exh_label_names, exh_labels = exh_reg_series.data(r=regions, return_regions=True)
    ...
    if create_val_projections:
        for p in tqdm(pat_ids, desc="Creating val projections for patients"):
            ...
            for i in range(n_val_volumes):
                ...
                # Sample kV detector angles from a uniform distribution.
                kv_detector_angles = list(np.random.uniform(min_angle, max_angle, n_val_angles))
                ...
                inh_ct_proj = F.interpolate(inh_ct_proj.unsqueeze(1).float(), size=VAL_SIZE, mode='bilinear', align_corners=False).squeeze(1)
                inh_labels_proj = F.interpolate(inh_labels_proj.float(), size=VAL_SIZE, mode='nearest')
                ...
                exh_ct_proj = F.interpolate(exh_ct_proj.unsqueeze(1).float(), size=VAL_SIZE, mode='bilinear', align_corners=False).squeeze(1)
                exh_labels_proj = F.interpolate(exh_labels_proj.float(), size=VAL_SIZE, mode='nearest')

                # Save each projection separately.
                # This means we can load at random for shuffled batches.
                for j, angle in enumerate(kv_detector_angles):
                    filepath = os.path.join(projpath, f"inh_ct_{i}_{j}.npy")
                    save_numpy(inh_ct_proj[j], filepath, overwrite=True)
                    filepath = os.path.join(projpath, f"exh_ct_{i}_{j}.npy")
...
if __name__ == '__main__':
    create_static_training_dataset(
        recreate=False,
        create_train_volumes=True,
        create_train_projections=True,

...
def predict(
    ...
    if register_images:
        ...
        os.makedirs(os.path.join(set.path, 'data', 'predictions', modelname, 'ct'), exist_ok=True)
        for movingpath, fixedpath in tqdm(lines):
            movedpath = os.path.join(set.path, 'data', 'predictions', modelname, 'ct', os.path.basename(movingpath))
            warppath = os.path.join(set.path, 'data', 'predictions', modelname, 'ct', os.path.basename(movingpath).replace('.nii.gz', '_warp.hdf5'))
    ...
    if register_labels:
        ...
        if regions is not None:
            ...
            for region in tqdm(regions):
                os.makedirs(os.path.join(set.path, 'data', 'predictions', modelname, 'regions', region), exist_ok=True)
                for movingpath, fixedpath in tqdm(lines, leave=False):
                    movinglabelpath = movingpath.replace('/ct/', f'/regions/{region}/')
                    movedlabelpath = os.path.join(set.path, 'data', 'predictions', modelname, 'regions', region, os.path.basename(movingpath))
                    warppath = os.path.join(set.path, 'data', 'predictions', modelname, 'ct', os.path.basename(movingpath).replace('.nii.gz', '_warp.hdf5'))
                    # Call voxelmorph script.
                    subprocess.run([
                        'unigradicon-warp',
    ...
    if register_landmarks:
        ...
        for movingpath, fixedpath in tqdm(lines):
            ...
            for point in fixed_lms:
                ...
                point_mm_no_dir = origin + point * spacing
                point_mm = origin + direction.dot(point * spacing)
            ...
            filepath = os.path.join(set.path, 'data', 'predictions', modelname, 'landmarks', f'{moving_pat_id}.csv')

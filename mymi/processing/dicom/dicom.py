...
from dicomset.dicom import recreate as recreate_dicom
...
def convert_to_dicom(
    ...
    if recreate_patient and os.path.exists(base_path):
    ...
    for p in tqdm(pat_ids):
        ...
        for s in studys:
            ...
            # Convert CT series.
            ct_serieses = dataset_fns['list_series'](study, 'ct')
            if len(ct_serieses) > 1:
                raise ValueError(f"Code only handles studies with a single CT series. See 'ct_dicoms' above.")
            ct_series = ct_serieses[0]
            # Load data.
            ct_series = dataset_fns['series'](study, ct_series, 'ct')
            ct_dicoms = to_ct_dicoms(ct_series.data, ct_series.spacing, ct_series.origin, p_mapped, s)
            for i, d in enumerate(ct_dicoms):
                if convert_ct:
                    filepath = os.path.join(base_path, p_mapped, s, 'ct', ct_series, f'{i:03d}.dcm')
                    os.makedirs(os.path.dirname(filepath), exist_ok=True)
                    d.save_as(filepath)

            # Convert regions to RTSTRUCT.
            # Don't convert landmarks as these should be in 'moving' space.
            if regions is not None and study.has_regions_data:
                all_serieses = list(np.unique(dataset_fns['list_series'](study, 'landmarks') + dataset_fns['list_series'](study, 'regions')))
                for ss in all_serieses:
                    ...
                    rtstruct_dicom = to_rtstruct_dicom(ct_dicoms, regions_data=r_data)
            ...
            if convert_dose and study.has_dose:
                dose_serieses = dataset_fns['list_series'](study, 'dose')
                for ss in dose_serieses:

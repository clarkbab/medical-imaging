...
from mymi.datasets.training_adaptive import TrainingAdaptiveDataset, exists as exists_training_adaptive
from mymi.datasets.training_adaptive import create as create_training_adaptive
from mymi.datasets.training_adaptive import recreate as recreate_training_adaptive
...
def convert_replan_adaptive_to_training(
    ...
    if create_data:
        for i, pat_id_mt in enumerate(tqdm(pat_ids_mt)):
            ...
            region_data_mt = pat_mt.region_data(region=regions, regions_ignore_missing=True)
            ...
            if crop_mm is not None:
                ...
                # Crop input.
                input_pt = crop(input_pt, crop)
                input_mt = crop(input_mt, crop)
                region_data_pt = dict((r, crop(d, crop)) for r, d in region_data_pt.items())
    ...
    if create_data:
        for i, pat_id in enumerate(tqdm(pat_ids)):
            ...
            if crop_mm is not None:
                ...
                input_pt = crop(input_pt, crop)
                input_mt = crop(input_mt, crop)
                region_data_input = dict((r, crop(d, crop)) for r, d in region_data_input.items())

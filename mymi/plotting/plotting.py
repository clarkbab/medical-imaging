...
def get_idx(
    ...
    if idx is not None:
        if centre is not None:
            if isinstance(centre, (LandmarkData, RegionData)):
                centre = type(centre)
        ...
        if isinstance(centre, str) and centre == 'dose':
            ...
            raise ValueError(f"Invalid type for 'centre': {type(centre)}. Must be one of (LandmarkData, LandmarkID, RegionData, RegionID).")
    else:

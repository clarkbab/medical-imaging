...
from mymi.transforms import crop, itk_load_transform, resample, sitk_load_transform, sitk_transform_image, sitk_transform_points
...
def create_patient_registration(
    ...
    sitk_transform = to_sitk_transform(dvf, fixed_spacing, fixed_offset)
    ...
    save_as_nifti(ct_moved, fixed_spacing, fixed_offset, filepath)
    ...
    if regions is not None:
        ...
        if moving_region_data is not None:
            for region, moving_label in moving_region_data.items():
                ...
                save_as_nifti(moved_label, fixed_spacing, fixed_offset, filepath)
...
def load_registration(
    ...
    if regions is not None:
        ...
        moved_region_data = {}
        for r in regions:
            rdata = load_registered_region(dataset, fixed_pat_id, model, r, fixed_study_id=fixed_study_id, moving_pat_id=moving_pat_id, moving_study_id=moving_study_id, raise_error=not regions_ignore_missing)
            if rdata is None:
                continue
    ...
    if landmarks is not None:
        ...
        moved_landmark_data = load_registered_landmarks(dataset, fixed_pat_id, model, fixed_study_id=fixed_study_id, moving_pat_id=moving_pat_id, moving_study_id=moving_study_id, raise_error=not regions_ignore_missing, use_image_coords=use_image_coords)

...
def predict_unigradicon(
    ...
    for p in tqdm(pat_ids):
        ...
        reg_path = os.path.join(set.path, 'data', 'predictions', 'registration', moving_pat_id, moving_study_id, fixed_pat_id, fixed_study_id, model)
        moved_path = os.path.join(reg_path, 'ct', 'series_0.nii.gz')
        transform_path = os.path.join(reg_path, 'dvf', 'series_0.hdf5')
        ...
        if regions is not None:
            ...
            for r in regions:
                ...
                moved_region_path = os.path.join(reg_path, 'regions', 'series_1', f'{r}.nii.gz')
        ...
        if landmarks is not None:
            ...
            lm_data = sitk_transform_points(lm_data, transform)
            moving_lm_df = fixed_lm_df.copy()
            moving_lm_df[list(range(3))] = lm_data

            # Save transformed points.
            filepath = os.path.join(reg_path, 'landmarks', 'series_1.csv')

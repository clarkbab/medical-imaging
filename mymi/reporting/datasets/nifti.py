...
def create_segmenter_prediction_figures(
    ...
    **kwargs) -> None:
    views = arg_to_list(view, str)
    logging.info(f"Creating segmenter prediction figures for dataset '{dataset}', region '{region}', test fold '{test_fold}', model '{model}' and view '{view}'.")
    ...
    for pat_desc_b in tqdm(iter(test_loader)):
        ...
        for pat_desc in pat_desc_b:
            ...
            for view, page_coord in zip(views, img_coords):
                # Add image to report.
                filepath = os.path.join(config.directories.temp, f'{uuid1().hex}.png')
                plot_segmenter_predictions(dataset, pat_id, model, model_region, centre=region, crop=region, savepath=filepath, show=False, show_legend=False, view=view, **kwargs)
                pdf.image(filepath, *page_coord, w=img_width, h=img_height)
                os.remove(filepath)
    ...
    # We have to 'encode' localisers/segmenters because they could be a list of models.
    set = NiftiDataset(dataset)
    filepath = os.path.join(NiftiDataset(set).path, 'reports', 'prediction-figures', 'segmenter', *model, f'figures-fold-{test_fold}.pdf')
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    pdf.output(filepath, 'F')

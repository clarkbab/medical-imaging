...
def get_region_summary(
    ...
    for pat_id in tqdm(pat_ids):
        ...
        label = pat.region_images(labels=labels, region=region)[region]
        ...
        if pat.has_regions('Brain'):
            ...
            brain_label = pat.region_images(region='Brain')['Brain']
...
def create_region_summary(
    ...
    for region in tqdm(regions):
        ...
        for pat_id in tqdm(pat_ids, leave=False):
            ...
            region_images = pat.region_images(regions=region)[region]
            if pat.has_regions('Brain'):
                brain_data = pat.region_images(regions='Brain')['Brain']
            else:
                brain_data = None

            # Get stats.
            stats = get_region_stats(ct_data, region_images, spacing, offset, brain_data)
            for stat in stats:
                data = {
                    'dataset': dataset,
                    'patient-id': pat_id,

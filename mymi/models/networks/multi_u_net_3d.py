...
class MultiUNet3D(nn.Module):
    def __init__(
        ...
        self.first_a = nn.Conv3d(in_channels=1, out_channels=n_features, kernel_size=3, stride=1, padding=1).to(self.__device_0)
        self.first_b = nn.InstanceNorm3d(n_features)
        self.first_c = nn.ReLU()
        self.first_d = nn.Conv3d(in_channels=n_features, out_channels=n_features, kernel_size=3, stride=1, padding=1).to(self.__device_0)
        ...
        self.down1_b = Conv(n_features, 2 * n_features).to(self.__device_1)
        self.down1_c = Conv(2 * n_features, 2 * n_features).to(self.__device_1)

        self.down2 = Down(2 * n_features, 4 * n_features).to(self.__device_1)
        self.down3 = Down(4 * n_features, 8 * n_features).to(self.__device_1)
        self.down4 = Down(8 * n_features, 16 * n_features).to(self.__device_1)
        self.up1 = Up(16 * n_features, 8 * n_features).to(self.__device_1)
        self.up2 = Up(8 * n_features, 4 * n_features).to(self.__device_1)

        # Split 'up3' to place on separate GPUs.
        self.up3_a = nn.ConvTranspose3d(in_channels=4 * n_features, out_channels=2 * n_features, kernel_size=2, stride=2).to(self.__device_1)
        self.up3_b = Conv(4 * n_features, 2 * n_features).to(self.__device_1)
        ...
        self.up4_d = nn.ReLU()
        self.up4_e = nn.Conv3d(in_channels=n_features, out_channels=n_features, kernel_size=3, stride=1, padding=1).to(self.__device_3)
        self.up4_f = nn.InstanceNorm3d(n_features)
        self.up4_g = nn.ReLU()

        self.out = OutConv(n_features, n_output_channels).to(self.__device_3)

...
from mymi.metrics import batch_mean, dice, distances
from mymi.models import replace_ckpt_alias
from mymi.models.architectures import UNet3D
...
class Segmenter(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if 'dice' in self.__metrics:
            dsc = batch_mean(dice, y_hat, y)
            self.log('val/dice', dsc, on_epoch=True, sync_dist=True)
            self.log(f"val/batch/dice/{descs[0]}", dsc, on_epoch=False, on_step=True)
        ...
        if 'distances' in self.__metrics and self.global_step > self.__distances_delay and self.current_epoch % self.__distances_interval == 0:
            if y_hat.sum() > 0 and y.sum() > 0:
                dists = batch_mean(distances, y_hat, y, self.__spacing)

...
from mymi.models.architectures import MultiUNet3DConvergence
from mymi.typing import ModelName, PatientID, PatientRegions
from mymi.utils import arg_to_list, gpu_usage_nvml

class MultiSegmenterConvergence(pl.LightningModule):
    def __init__(
        self,
        ...
        **kwargs) -> None:
        ...
        self.__network = MultiUNet3DConvergence(self.__n_output_channels, **kwargs)
    ...
    def load(
        ...
        **kwargs: Dict) -> pl.LightningModule:
        ...
        for k, v in ckpt_data['state_dict'].items():
            # Get new key.
            if k.startswith('_Segmenter__MultiSegmenterConvergence__network'):
                update = True
                new_k = k.replace('_Segmenter__MultiSegmenterConvergence__network', '_MultiSegmenterConvergence__network')
            else:
                new_k = k
        ...
        # Load checkpoint.
        segmenter = MultiSegmenterConvergence.load_from_checkpoint(filepath, cw_cvg_calculate=cw_cvg_calculate, **kwargs)
    ...
    def configure_optimizers(self):
        ...
        elif self.__optimiser == 'rmsprop':
            self.__optimiser = RMSprop(self.parameters(),lr = self.lr)
        elif self.__optimiser == 'sgd':
            return SGD(self.parameters(), lr=self.lr, momentum=0.9)
        else:
            raise ValueError(f"Optimiser '{self.__optimiser}' not recognised.")

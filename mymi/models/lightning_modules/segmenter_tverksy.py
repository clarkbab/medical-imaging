...
class SegmenterTversky(pl.LightningModule):
    def __init__(
        self,
        regions: Regions,
        arch: str = 'unet3d:m',
        ...
        **kwargs) -> None:
        ...
        self.__loss_fn = DynamicTverskyLoss(smoothing=loss_smoothing)
        self.lr = lr_init        # 'self.lr' is default key that LR finder looks for on module.
        self.__tversky_lr_init = tversky_lr_init
        self.__name = name
        self.__regions = regions
        self.__n_channels = len(self.__regions) + 1
        self.__tversky_params = torch.nn.Parameter(0.5 * torch.ones((2, self.__n_channels), dtype=torch.float32))
        if arch == 'unet3d:m':
    ...
    def configure_optimizers(self):
        ...
        self.__tversky_optimiser = torch.optim.AdamW([self.__tversky_params], lr=self.__tversky_lr_init)
        logging.info(f"Using tversky optimiser '{self.__tversky_optimiser}' with learning rate '{self.__tversky_lr_init}'.")
        opt = {
    ...
    def training_step(self, batch, _):
        ...
        soft_params = torch.nn.functional.softmax(self.__tversky_params, dim=0)
        loss, loss_tversky = self.__loss_fn(y_hat, y, soft_params, **kwargs) 
        self.__loss_tversky = loss_tversky

        if self.logger is not None and self.__interval_matches(self.__wandb_train_metrics_record_interval):
            self.log(f'train/loss-tversky', loss_tversky, on_epoch=self.__wandb_log_on_epoch, on_step=not self.__wandb_log_on_epoch)
        ...
        if self.__save_training_metrics and self.__interval_matches(self.__train_metrics_record_interval):
            data = {
                ...
                'shape': '',
                'metric': 'loss-tversky',
                'value': loss_tversky.item(),
            }
            self.__training_metrics = append_row(self.__training_metrics, data)

        for i, r in enumerate(self.__regions):
            if self.logger is not None and self.__interval_matches(self.__wandb_train_metrics_record_interval):
                self.log(f'train/tversky-alpha/{r}', soft_params[0][i + 1], on_epoch=self.__wandb_log_on_epoch, on_step=not self.__wandb_log_on_epoch)
            ...
            if self.__save_training_metrics and self.__interval_matches(self.__train_metrics_record_interval):
                data = {
                    ...
                    'module-type': '',
                    'shape': '',
                    'metric': 'tversky-alpha',
                    'value': soft_params[0][i + 1].item(),
                }
                self.__training_metrics = append_row(self.__training_metrics, data)

            if self.logger is not None and self.__interval_matches(self.__wandb_train_metrics_record_interval):
                self.log(f'train/tversky-beta/{r}', soft_params[1][i + 1], on_epoch=self.__wandb_log_on_epoch, on_step=not self.__wandb_log_on_epoch)
    ...
    def backward(
        ...
        self.__tversky_optimiser.zero_grad()
        grad = torch.autograd.grad(self.__loss_tversky, self.__tversky_params)[0]
        self.__tversky_params.grad = grad
        self.__tversky_optimiser.step()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        soft_params = torch.nn.functional.softmax(self.__tversky_params, dim=0)
        loss, _ = self.__loss_fn(y_hat, y, soft_params, **kwargs) 

...
class BSPUNet2DPhase(nn.Module):
    """BSPUNet2D with sin/cos encoding for the phase signal.

    Outputs 3 channels:
        0 - amplitude (min-max normalised to [0, 1])
        1 - sin(2π * phase) (tanh → [-1, 1])
        2 - cos(2π * phase) (tanh → [-1, 1])

    Decode predicted phase:
        phase = (atan2(pred[:, 1], pred[:, 2]) / (2π)) % 1.0
    """
    ...
    def __init__(
        ...
        # Fully-connected layer: per-X-position projection from (C_final * Y_reduced) → 3.
        # Channel 0: amplitude,  channels 1–2: sin/cos of phase.
        c_final = 2 ** n_levels * n_features
        y_reduced = y_size // (2 ** n_levels)
        self.__fc = nn.Conv1d(c_final * y_reduced, 3, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Per-position FC: (B, 3, X)
        x = self.__fc(x)

        # Amplitude channel: min-max normalise to [0, 1].
        amp = x[:, :1, :]  # (B, 1, X)
        amp_min = amp.amin(dim=2, keepdim=True)
        amp_max = amp.amax(dim=2, keepdim=True)
        amp = (amp - amp_min) / (amp_max - amp_min + 1e-8)

        # Sin/cos channels: tanh to [-1, 1].
        sincos = torch.tanh(x[:, 1:, :])  # (B, 2, X)

        return torch.cat([amp, sincos], dim=1)  # (B, 3, X)
